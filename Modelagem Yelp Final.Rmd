...
recommendation %>% 
    ...
    ggplot(aes(x = latitude, y = longitude, color = rank)) +
    geom_point()+
...
recommendation %>% 
  ...
  filter(category %in% c('food', 'restaurants','pub','bar','club')) %>% 
  group_by(category) %>%
  mutate(pred_avg = mean(V1)) %>% 
  select(name, V1, category, pred_avg) %>% 
  arrange(-pred_avg) %>% 
  top_n(n = 5, wt = V1) %>% 
