...
reviewed_usr <- tibble(business_id = sample(yelp_bz_raw$business_id, n_reviews),
                                            ...
                                            year_rv = ceiling(runif(1,2009, 2019)),
                                            )
user_hist <- user %>% 
...
to_review <- user_hist %>% 
             bind_rows(to_go) %>% 
             #mutate(stars_rv, replace(., is.na(.), 0)) %>% 
             na.locf() #preenche linhas vazias com informação do usuário
...
predictions <- yelp_nn %>% 
  ...
  user_y_test <- as.numeric(to_review$stars_rv)
  
  results <- yelp_nn %>% 
    evaluate(user_x_test, user_y_test)

  recommendation <- to_go %>% 
    bind_cols(pred = predictions) %>% 
    sort(pred, decreasing = TRUE) %>% 
    anti_join(user_hist$business_id, by = 'business_id')
