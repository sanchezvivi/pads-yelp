...
auxiliar <- tibble(cluster = yelp_km$cluster) %>% 
              bind_cols(as_tibble(yelp_juice))
...
criterio <- function(k) kmeans(yelp_km, k)$tot.withinss


estudo <- tibble(k = 1:50) %>% #estudo até 15 clusters
...
res_silhouette <- silhouette(kmedias$cluster, dist(yelp_juice))
...
dados %>% 
  mutate(cluster = as.factor(cluster)) %>% 
  ggplot(aes(cluster, total_litres_alcohol)) + 
